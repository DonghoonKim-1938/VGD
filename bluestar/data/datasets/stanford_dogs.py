...
class StanfordDogs(DatasetBase):
    ...
    def __init__(
        ...
        # stanford_dogs = ImageFolder(data_path)

        # imagenet = ImageFolder(data_dir + "/train" if train else "/val" ) else 2012
        # self.data = [i[0] for i in stanford_dogs.samples]  # list of str (file paths)
        # self.targets = [i[1] for i in stanford_dogs.samples]  # list of int (labels)
    ...
    def __getitem__(self, index) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        img = self.data_dir + img.replace('stanford_dogs', '')
        img = self.loader(img)
